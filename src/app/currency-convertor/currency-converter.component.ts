...
export class CurrencyConverterComponent implements OnInit{
  ...
  constructor(private _fb:FormBuilder,private _dialog:MatDialog,private _converterService:ConverterService){}

  private _exchanges:Exchange[] = [];
  private _dialogRef:MatDialogRef<AddExchangeComponent> | null = null;
  private $userInput = new Subject<{from:string,to:string,value:string,index:number}>();
  histories:string[] = [];
  ...
  ngOnInit(): void {
    this.$userInput.pipe(
      switchMap(
      data => {
        return this._converterService.exchange(data.from,data.to,data.value,data.index)}
    )).subscribe(
      (result:any) => {
        let formElement =  this.currencyForm.controls.currencies.at(result.index);
        ...
        this.histories.push(`base currency: ${formElement.controls['fromLabel'].value} `+formElement.controls['toLabel'].value+' to: '+formElement.controls['from'].value+' = ' + result.value);
      }
    )
  }
